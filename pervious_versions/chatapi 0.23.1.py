...
try:
    ...
    try:
        ...
        sys.exit(0)
try:
    from GptToCad import CADCommandProcessor
...
with open('think_img.jpg', 'wb') as f:
    f.write(think_img)
# 全局变量
global sysrule
...
chathistory = []
# 初始化聊天历史，添加系统规则
chathistory.append({'role': 'system', 'content': sysrule})
...
class ChatapiTextBrowser(QTextBrowser):
    ...
    def highlight_code(code: str, lang: str, code_formatter) -> str:
        ...
        try:
            ...
            return f'<pre style="color:red">Error: {html.escape(str(e))}</pre>'
...
class APIConfigDialogUpdateModelThread(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            update_ollama = ModelListUpdater.is_ollama_alive()
            ModelListUpdater.update_model_map(update_ollama=update_ollama)
...
class MessagePreprocessor:
    ...
    def prepare_message(self,tools=False):
        ...
        message = self._insert_recent_user_message(message, better_round)
    ...
    def _insert_recent_user_message(self, message, better_round):
        """插入最近的用户消息"""
        if len(message) >= 2 and message[1].get("role") != "user":
            start, end = 1, len(self.god.chathistory) - (better_round-1)
            if start <= end and end >= 0:
                found = next((elem for elem in reversed(
                    self.god.chathistory[start:end]) if elem["role"] == "user"), None)
                if found:
                    message.insert(1, dict(found))
        return message
...
class ResponseProcessor:
    ...
    def process_stream(self, response_stream):
        ...
        for event in response_stream:
            if self.parent.pause_flag:  # 访问原类的暂停标志
                return
    ...
    def _extract_content(self, event):
        """从事件中提取有效内容"""
        if not hasattr(event, "choices") or not event.choices:
            return None
        return getattr(event.choices[0], "delta", None)
    
    def _process_content(self, content):
        """处理不同类型的内容"""
        # 处理常规内容
        if hasattr(content, "content") and content.content:
            self.temp_response += content.content
            self._handle_regular_content(content.content)
        
        # 处理思考链内容（假设这是自定义扩展字段）
        if hasattr(content, "reasoning_content") and content.reasoning_content:
            self._handle_reasoning_content(content.reasoning_content)
    
    def _handle_regular_content(self, content):
        """处理普通文本内容"""
        # 使用原工具类处理特殊块
        handler_result = StrTools.special_block_handler(
            self.parent,  # 保持与原工具类的兼容性
            self.temp_response,
            self.parent.think_response_signal.emit,
            starter='<think>',
            ender='</think>',
        ...
        # 根据处理结果更新响应
        if handler_result["starter"] and handler_result["ender"]:
            self.full_response += content
            self.full_response = self.full_response.replace('</think>\n\n', '')
            self.parent.ai_response_signal.emit(self.full_response)
        elif not handler_result["starter"]:
            self.full_response += content
            self.parent.ai_response_signal.emit(self.full_response)
    
    def _handle_reasoning_content(self, content):
        """处理思考链内容"""
        self.think_response += content
        self.parent.think_response_signal.emit(self.think_response)
    
    def _finalize_processing(self):
        """最终处理步骤"""
        print(f'\n返回长度：{len(self.full_response)}\n思考链长度: {len(self.think_response)}')
        self.parent.update_response_signal.emit(self.full_response)
...
class FunctionLibrary:
            ...
            "description": [
    {
        ...
        "function": {
            ...
            "definition": FunctionLibrary.weather_test
        },
        {
            "name": "open_file",
            "description": [
...
"}\n"
    ...
    def __init__(self):
        ...
        #背景
        self.init_back_ground_label('think_img.jpg')
        #更换函数
        #self.switchImage("599065.jpg")
        #self.test_back=QPushButton("强制更新背景")
        #self.test_back.clicked.connect(self.back_ground_update)
        #self.main_layout.addWidget(self.test_back,0,2,1,1)

        #button_tts=QPushButton("tts")
        #button_tts.clicked.connect(self.save_chat_to_speech)
        #self.main_layout.addWidget(button_tts,0,20,1,1)


        model_label = QLabel("选择模型:")
        self.model_combobox = QComboBox()
        self.model_combobox.addItems(MODEL_MAP.keys())
        self.model_combobox.setCurrentText("deepseek-r1")  # 默认值
        ...
        #self.main_layout.addWidget(self.stat_tab_widget,0,  2,  1,  1)
...
QPushButton:pressed {
    ...
    def perform_ai_actual_update(self):
        # 更新界面和滚动条
        actual_response = self.full_response
        if self.autoreplace_var:
            actual_response = StrTools.vast_replace(actual_response,self.autoreplace_from,self.autoreplace_to)
    ...
    def perform_think_actual_update(self):
        ...
        self.think_text_box.setMarkdown(self.think_response)
    ...
    def receive_message(self, content):
        ...
        try:
            ...
            if content.startswith("\n\n"):
                content = content[2:]
            if self.autoreplace_var:
                content = StrTools.vast_replace(content,self.autoreplace_from,self.autoreplace_to)
            self.chathistory.append({'role': 'assistant', 'content': content})
            self.save_chat_to_speech()
            # 发出信号，通知主线程更新界面
            self.ai_response_text.setMarkdown(self.full_response)
    ...
    def autocad_drawing(self):
        global sysrule, chathistory
        try:
            self.sysrule=sysrule = cad_processor.sysrules()  # 获取系统规则
...
'''     
    ...
    def save_chat_to_speech(self):
        ...
        content_to_save=extract_dialogue(self.chathistory[-1]["content"])
        file_to_save=r'D:\cosyvoice\CosyVoice\chatapi\from_chat_api.txt'
        with open(file_to_save, "w",encoding="utf-8") as audio_text:
            audio_text.write(content_to_save)
