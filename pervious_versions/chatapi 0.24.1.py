...
try :
    from mods.status_monitor import StatusMonitorWindow
except ImportError as e:
...
class ModConfiger(QTabWidget):
    ...
    def add_status_monitor(self):
        ...
        self.name_ai_label = QLabel("指定状态栏挂载角色")
        status_monitor_layout.addWidget(self.name_ai_label, 3, 0, 1, 1)
        self.name_ai_line_edit = QLineEdit()
        status_monitor_layout.addWidget(self.name_ai_line_edit, 3, 1, 1, 1)
        self.name_ai_line_edit.setPlaceholderText("请输入角色名称")
        self.name_ai_line_edit.setToolTip("如果没有指定，默认使用AI当前名字,打开管理器以后也可以编辑")
        self.name_ai_line_edit.textChanged.connect(lambda text: setattr(self.status_monitor, 'name_ai', text))
