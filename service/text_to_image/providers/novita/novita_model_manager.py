...
class NovitaModelManager:
    ...
    @staticmethod
    def extract_model_names(result) -> list:
        """从 Novita 模型接口响应中提取模型名称"""
        if not isinstance(result, dict):
            return []
        data = result.get('data') or result.get('models')
        if not isinstance(data, list):
            return []
        names = [item.get('model_name') for item in data if isinstance(item, dict) and item.get('model_name')]
        return names
...
class NovitaModelFetcher:
    update_done = Signal(list)

    def __init__(self, application_path='', api_key=''):
        self.base_url = "https://api.novita.ai/v3/model?filter.types=checkpoint"
        self.headers = {"Authorization": f"Bearer {api_key}"}
        self.application_path = application_path
        self.model_list = NovitaModelManager(application_path).get_model_options()
    ...
    def _fetch_in_thread(self):
        try:
            ...
            if 'data' not in result and 'models' not in result:
                raise ValueError(f"API响应缺少'data'或'models'字段, response={result}")

            new_list = NovitaModelManager.extract_model_names(result)
...
class NovitaAgent:
    ...
    def __init__(self, api_key, application_path, save_folder='pics'):
        ...
        self.model_updater = NovitaModelFetcher(application_path=application_path, api_key=api_key)
