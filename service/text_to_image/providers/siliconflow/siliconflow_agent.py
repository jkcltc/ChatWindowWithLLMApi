...
class SiliconflowModelFetcher:
    update_done = Signal(list)

    def __init__(self, application_path='', api_key=''):
        self.exact_url = 'https://api.siliconflow.cn/v1/models?sub_type=text-to-image'
        self.universal_url = 'https://api.siliconflow.cn/v1/models'
        self.headers = {"Authorization": f"Bearer {api_key}"}
        self.application_path = application_path
        self.model_list = SiliconflowModelManager(application_path).get_model_options()  # 共享资源需要保护
...
class SiliconflowModelManager:
    ...
    def __init__(self, application_path=''):
        self.file_path = os.path.join(application_path, 'service', 'text_to_image', 'providers', 'siliconflow', 'SILICON_IMAGE_MODELS.json')
...
class SiliconFlowAgent:
    ...
    def __init__(self, api_key, application_path, save_folder='data/pics', parent=None):
        ...
        self.generator.failure.connect(print)
