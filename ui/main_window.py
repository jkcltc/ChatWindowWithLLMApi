...
start_log(
    f"CWLA config recover finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)
...
start_log(
    f"CWLA service import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)

from core.session.title_generate import TitleGenerator

start_log(
    f"CWLA core title import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)

from core.background import BackgroundAgent

start_log(
    f"CWLA core multimodal_coordination import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)

from core.app_core import CWLACore

start_log(
    f"CWLA core import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)
...
from ui.setting.mcp_config_widget import MCPConfigWidget
...
from ui.chat.chathistory_view_widget import (
    ChatapiTextBrowser,
    ChatHistoryWidget,
    ChatHistoryTextView,
    HistoryListWidget,
...
start_log(
    f"CWLA utils import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)
...
class FPSMonitor(QLabel):
    ...
    def _update_display(self):
        ...
        self.setText(
            f"FPS: {fps:02d} | Lag: <span style='color:{color}'>{lag_ms}ms</span>"
        )
        self.adjustSize()
...
class MainWindow(MainWindow):
    ...
    def __init__(self):
        ...
        self.use_concurrent_model.setToolTip(
            "用于提高生成质量\n注意！！极高token消耗量！！"
        )
        self.use_concurrent_model.toggled.connect(
            lambda checked: self.show_concurrent_model(show=checked)
        )

        # 两模式互斥
        self.use_muti_model.toggled.connect(
            lambda c: self.use_concurrent_model.setChecked(False) if c else None
        )
        self.use_muti_model.toggled.connect(
            lambda c: APP_SETTINGS.model_poll.update({"enabled": c})
        )
        self.use_concurrent_model.toggled.connect(
            lambda c: self.use_muti_model.setChecked(False) if c else None
        )
        self.use_concurrent_model.toggled.connect(
            lambda c: APP_SETTINGS.concurrent.update({"enabled": c})
        )
        ...
        self.opti_frame_layout.addWidget(
            self.cancel_trigger_background_update, 0, 8, 1, 1
        )
        ...
        self.stat_tab_widget.setSizePolicy(
        ...
        temp_style_edit.textChanged.connect(
            lambda text: setattr(self, "temp_style", text or "")
        )
        ...
        self.ai_response_text.anchorClicked.connect(
            lambda url: os.startfile(url.toString())
        )
        ...
        self.pause_button.clicked.connect(
            lambda: self.control_frame_to_state("finished")
        )
        self.pause_button.clicked.connect(
            lambda _: self.chat_history_bubbles.streaming_scroll(False)
        )
        ...
        self.search_result_button.clicked.connect(
        ...
        self.web_search_button = ExpandableButton(
            [
                "搜索：关闭",
                "搜索：自动",
                "搜索：强制",
        ...
        self.web_search_button.indexChanged.connect(
            self.handle_web_search_button_index_changed
        )
        if APP_SETTINGS.web_search.web_search_enabled:
            self.web_search_button.setCurrentIndex(2)

        self.enable_thinking_button = ExpandableButton(
            ["深度思考", "思考：短", "思考：中", "思考：高"]
        )
        self.enable_thinking_button.setChecked(APP_SETTINGS.generation.thinking_enabled)
        self.enable_thinking_button.setCurrentIndex(
            APP_SETTINGS.generation.reasoning_effort
        )
        self.enable_thinking_button.toggled.connect(
            lambda state: APP_SETTINGS.generation.update({"thinking_enabled": state})
        )
        ...
        self.past_chat_list.setSelectionMode(
...
""")
        ...
        self.tree_view.setGeometry(
            -int(self.width() * 0.3), 0, int(self.width() * 0.3), int(self.height())
        )
        ...
        self.toggle_tree_button.setGeometry(
            0,
            self.height() - int(self.height() * 0.06),
        ...
        self.info_manager.log(
            f"CWLA init finished, time stamp:{time.time() - start_time_stamp:.2f}s"
        )
    ...
    def connect_bus_signals(self):
        ...
        b.tool_changed.connect(
            lambda id, tool: self.function_manager.set_active_tools(tool)
        )
    ...
    def init_function_window(self):
        ...
        self.function_manager.activatedToolsChanged.connect(
            self.session_manager.set_tools
        )
    ...
    def init_system_prompt_window(self):
        ...
        self.system_prompt_override_window.update_preset.connect(
            self.update_system_preset
        )
    ...
    def init_chat_history_bubbles(self):
        ...
        self.display_full_chat_history.clicked.connect(
            self.display_full_chat_history_window
        )
        self.chat_history_text = ChatapiTextBrowser()
        self.chat_history_text.anchorClicked.connect(
            lambda url: os.startfile(url.toString())
        )
    ...
    def add_tts_page(self):
        self.tts_handler = TTSAgent(
            setting=APP_SETTINGS.tts,
    ...
    def resizeEvent(self, event):
        ...
        self.past_chat_frame.setGeometry(
            self.width() - self.past_chat_frame.width(),
            0,
            int(self.width() * 0.3),
            int(self.height()),
        )
    ...
    def changeEvent(self, event):
        try:
            ...
            self.tree_view.setGeometry(
                0, 0, int(self.width() * 0.3), int(self.height())
            )
        except Exception as e:
        ...
        data = [
            {
                "上级名称": "系统",
                "提示语": "API/模型库设置",
                "执行函数": self.open_api_window,
            },
            {
                "上级名称": "系统",
                "提示语": "MCP设置",
                "执行函数": self.open_mcp_window,
            },
            {
                "上级名称": "系统",
                "提示语": "System Prompt 设定 Ctrl+E",
                "执行函数": self.open_system_prompt,
            },
            {
                "上级名称": "系统",
                "提示语": "MOD管理器",
                "执行函数": self.show_mod_configer,
            },
            {
                "上级名称": "记录",
                "提示语": "保存记录",
                "执行函数": self.save_chathistory,
            },
            {
                "上级名称": "记录",
                "提示语": "导入记录",
                "执行函数": self.load_chathistory,
            },
            {
                "上级名称": "记录",
                "提示语": "修改原始记录",
                "执行函数": self.edit_chathistory,
            },
            {
                "上级名称": "记录",
                "提示语": "对话分析",
                "执行函数": self.show_analysis_window,
            },
            {
                "上级名称": "对话",
                "提示语": "强制触发长对话优化",
                "执行函数": self.cfm.enforce_lci,
            },
            {
                "上级名称": "对话",
                "提示语": "函数调用",
                "执行函数": self.show_function_call_window,
            },
            {
                "上级名称": "背景",
                "提示语": "背景设置",
                "执行函数": self.background_settings_window,
            },
            {
                "上级名称": "背景",
                "提示语": "触发背景更新（跟随聊天）",
                "执行函数": self.call_background_update,
            },
            {
                "上级名称": "背景",
                "提示语": "生成自定义背景（正在重构）",
                "执行函数": self.show_pic_creater,
            },
            {
                "上级名称": "设置",
                "提示语": "对话设置",
                "执行函数": self.open_main_setting_window,
            },
            {
                "上级名称": "设置",
                "提示语": "快捷键",
                "执行函数": self.open_settings_window,
            },
            {
                "上级名称": "设置",
                "提示语": "联网搜索",
                "执行函数": self.open_web_search_setting_window,
            },
        ]
    ...
    def toggle_tree_view(self):
        ...
        if self.tree_view.isHidden():
            self.past_chat_frame.setGeometry(
                self.width(), 0, int(self.width() * 0.3), int(self.height())
            )
            self.past_chat_frame.show()
            self.past_chat_frame_animation = QPropertyAnimation(
                self.past_chat_frame, b"geometry"
            )
            self.past_chat_frame_animation.setDuration(300)
            self.past_chat_frame_animation.setEasingCurve(QEasingCurve.Type.InOutQuad)
            self.past_chat_frame_animation.setStartValue(
                QRect(self.width(), 0, self.past_chat_frame.width(), self.height())
            )
            self.past_chat_frame_animation.setEndValue(
                QRect(
                    self.width() - self.past_chat_frame.width(),
                    0,
                    self.past_chat_frame.width(),
                    self.height(),
                )
            )
            ...
            self.tree_animation.setStartValue(
                QRect(-self.tree_view.width(), 0, self.tree_view.width(), self.height())
            )
            self.tree_animation.setEndValue(
                QRect(0, 0, self.tree_view.width(), self.height())
            )

            # 创建 toggle_tree_button 的动画
            self.button_animation = QPropertyAnimation(
                self.toggle_tree_button, b"geometry"
            )
            ...
            self.button_animation.setEndValue(
                QRect(
                    self.tree_view.width(),
                    self.toggle_tree_button.y(),
                    self.toggle_tree_button.width(),
                    self.toggle_tree_button.height(),
                )
            )
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.MouseButtonPress:
            if self.tree_view.isVisible():
                # 将全局坐标转换为树视图的局部坐标
                local_pos = self.tree_view.mapFromGlobal(
    ...
    def display_full_chat_history_window(self):
        self.history_text_view = ChatHistoryTextView(self.session_manager.history)
    ...
    def _update_buffer(self, req_id, key, text):
        ...
        self._cb_buffers.role = (
            "assistant" if key in ["content", "reasoning"] else "tool"
        )
    ...
    def control_frame_to_state(self, state: bool | str):
        if not isinstance(state, bool):
            state_map = {"sending": False, "finished": True}
    ...
    def _get_current_llm_usage(self) -> LLMUsagePack:
        ...
        return LLMUsagePack(
            provider=self.api_var.currentText(), model=self.model_combobox.currentText()
        )
    ...
    def resend_message(self, request_id=""):
        ...
        success = self.cfm.resend_message(
            msg_id=request_id, LLM_usage=llm_usage, temp_style=self.temp_style
        )
    ...
    def send_message(self):
        ...
        if self.session_manager.get_last_message().get("role", "") == "user":
            ...
            btn_edit = msg_box.addButton(
                "编辑聊天记录", QMessageBox.ButtonRole.ActionRole
            )
    ...
    def open_mcp_window(self):
        if not hasattr(self, "mcp_window"):
            self.mcp_window = MCPConfigWidget()
            self.mcp_window.notificationRequested.connect(self.info_manager.notify)
        self.mcp_window.show()
        self.mcp_window.raise_()
    ...
    def load_chathistory(self, file_path=None):
        if not file_path:
            file_path, _ = QFileDialog.getOpenFileName(
                self, "选择聊天记录文件", "", "JSON Files (*.json);;All Files (*)"
            )
            if not file_path:
...
对话标识 {self.session_manager.chat_id}"""
    ...
    def edit_chathistory(self, file_path=""):
        ...
        self._ch_editor = ChatHistoryEditor(
            title_generator=TitleGenerator(
                api_handler=APIRequestHandler(), settings=APP_SETTINGS.title
            ),
            session=session,
        )
    ...
    def save_chathistory(self):
        ...
        success = self.session_manager.fork_chathistory(
            file_path=file_path, chat_session=self.session_manager.current_chat
        )
    ...
    def grab_past_chats(self):
        self.past_chat_list.populate_history(self.session_manager.past_session_list)
    ...
    def load_from_past(self, index):
        if not self.past_chat_list.currentItem():
            ...
            return
        self.load_chathistory(file_path=self.past_chat_list.get_selected_file_path())
    ...
    def _connect_setting_signals(self):
        ...
        self.main_setting_window.background_setting_changed.connect(
            self.update_opti_bar
        )
        self.main_setting_window.background_preview_changed.connect(
            self.update_background
        )
        self.main_setting_window.name_changed.connect(
            self.session_manager.set_role_name
        )
    ...
    def past_chats_menu(self, position):
        ...
        delete_action.triggered.connect(lambda: self.delete_selected_history())

        import_action = context_menu.addAction("导入system prompt")
        import_action.setToolTip(
            "从过去的中获取系统提示文本并覆盖掉\n当前对话中的系统提示。"
        )
        import_action.triggered.connect(lambda: self.load_sys_pmt_from_past_record())

        world_view_action = context_menu.addAction("分析")
        world_view_action.setToolTip("打开分析窗口")
        world_view_action.triggered.connect(lambda: self.analysis_past_chat())
    ...
    def load_sys_pmt_from_past_record(self):
        file_path = self.past_chat_list.get_selected_file_path()
        sys_pmt = self.session_manager.load_sys_pmt_from_past_record(
            file_path=file_path
        )
        if sys_pmt:
    ...
    def update_background(self, file_path):
        ...
        if not file_path or not os.path.isfile(fp):
            QMessageBox.critical(
                None, "背景更新", "获取的图像路径无效", QMessageBox.StandardButton.Ok
            )
            return
    ...
    def bind_background_signals(self):
        ...
        add_connection(self.background_agent.signals.log, self.info_manager.log)
        add_connection(self.background_agent.signals.warning, self.info_manager.warning)
        add_connection(self.background_agent.signals.error, self.info_manager.error)
        add_connection(
            self.background_agent.signals.poll_success,
            lambda path: (
                [
                    self.update_background(path)
                    or self.info_manager.log(f"背景生成返回了路径，返回了{path}")
                ]
                if path
                else [
                    self.update_background("background.jpg"),
                    self.info_manager.log(f"背景生成没返回路径，返回了{path}"),
                ]
            ),
        )
    ...
    def open_background_pic(self):
        os.startfile(
            os.path.join(
                APP_RUNTIME.paths.application_path, APP_SETTINGS.background.image_path
            )
        )
    ...
    def update_opti_bar(self, _=None):
        ...
        self.cancel_trigger_background_update.setVisible(
            APP_SETTINGS.background.enabled
        )
        ...
        if ncr >= APP_SETTINGS.lci.max_segment_rounds:
            ...
            self.chat_opti_trigger_bar.setFormat(
                f"对话优化: {ncr}/{APP_SETTINGS.generation.max_message_rounds}"
            )
        if nbr >= APP_SETTINGS.generation.max_message_rounds:
            self.Background_trigger_bar.setFormat(f"背景更新: 即将触发")
        else:
            self.Background_trigger_bar.setFormat(
                f"背景更新: {nbr}/{APP_SETTINGS.background.max_rounds}"
            )
        self.opti_frame.setVisible(
            APP_SETTINGS.background.enabled or APP_SETTINGS.lci.enabled
        )
    ...
    def handle_search_result_button_toggle(self):
        ...
        if self.search_result_button.isChecked():
            ...
            self.main_layout.addWidget(
                self.web_searcher.search_results_widget, 3, 2, 4, 1
            )
            self.main_layout.setColumnStretch(0, 1)
    ...
    def handle_web_search_button_index_changed(self, index):
        ...
        if index in [0, 2]:
            selected_functions = self.session_manager.tools
            selected_functions = [
                func for func in selected_functions if func != "web_search"
            ]
    ...
    def show_avatar_window(self, msg_id, name):
        ...
        avatar_info = {
            "user": {"name": name_user, "image": avatar_user},
        ...
        if not hasattr(self, "avatar_creator"):
            ...
            self.avatar_creator.avatar_info["user"] != name_user
            or self.avatar_creator.avatar_info["assistant"] != name_ai
        ):
            do_init = True
        if do_init:
            ...
            self.avatar_creator.avatarCreated.connect(
                self.session_manager.set_role_avatar
            )
...
start_log(
    f"CWLA Class import finished, time stamp:{time.time() - start_time_stamp:.2f}s"
)
...
def start():
    ...
    window.show()
    start_log(
        f"CWLA shown on desktop, time stamp:{time.time() - start_time_stamp:.2f}s"
    )
    sp.close()
    exit_code = 0
    try:
