...
from PyQt6.QtWidgets import (
    ...
    QLineEdit,QSlider,QRadioButton,QGridLayout,
...
from .background import BackgroundSettingsWidget
from .lci import LongChatSettingsWidget
from .theme_manager import ThemeSelector
# 简易小组件
from ui.custom_widget import QuickSeparator

class MainSettingWindow(QWidget):
    lci_enabled_changed = pyqtSignal(bool)
    background_setting_changed = pyqtSignal(bool)
    background_preview_changed = pyqtSignal(str)
    ...
    def create_background_tab(self):
        """创建背景设置选项卡 - 嵌入 BackgroundSettingsWidget"""
        self.background_widget = BackgroundSettingsWidget(self)
        self.background_widget.updateSettingChanged.connect(self.background_setting_changed)
        self.background_widget.previewImageChanged.connect(self.background_preview_changed)
        self.background_widget.updateImageModelRequested.connect(self._refresh_background_image_models)
        self.background_widget.updateModelRequested.connect(self._refresh_background_text_models)

        self._load_background_image_models()
        self.tab_widget.addTab(self.background_widget, "背景设置")
    ...
    def _load_background_image_models(self):
        """加载背景图像模型列表（不触发远端更新）。"""
        image_agent = ImageAgent()
        image_model_map = image_agent.get_image_model_map()
        self.background_widget.set_image_model_map(image_model_map)

    def _refresh_background_image_models(self):
        """刷新背景图像模型列表（触发远端更新）。"""
        image_agent = ImageAgent()
        image_agent.update_models_async(self.background_widget.set_image_model_map)

    def _refresh_background_text_models(self):
        """刷新背景文本模型列表（summary 模型）供 BackgroundSettingsWidget 使用。"""
        self.background_widget.load_from_settings()
